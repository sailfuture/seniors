...
import { fetchBtSections, findBtSectionBySlug, btSlugToTitle } from "@/lib/businessthesis-sections"
import { BUSINESSTHESIS_API_CONFIG } from "@/lib/form-api-config"

export default function BusinessThesisDynamicSectionPage({
  params,
}: {
  ...
  const [title, setTitle] = useState(btSlugToTitle(section))
  const [description, setDescription] = useState<string | undefined>(undefined)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelled = false
    fetchBtSections().then((sections) => {
      if (cancelled) return
      const match = findBtSectionBySlug(sections, section)
      if (match) {
        setSectionId(match.id)
        setTitle(match.section_title)
        setDescription(match.description || undefined)
      }
      setLoading(false)
    })
