...
const COMMENTS_ENDPOINT = `${BT_BASE}/businessthesis_comments`
...
export default function StudentBusinessThesisOverviewPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result: SectionRow[] = sorted.map((s) => ({
        ...
        groups: groups.filter((g) => g.businessthesis_sections_id === s.id).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)),
      }))
  ...
  const sheetComments = sheetRow
    ? comments.filter((c) => {
        ...
        if (Number(c.businessthesis_sections_id) !== sheetRow.section.id) return false
        if (sheetGroupId !== null) {
          return Number(c.businessthesis_custom_group_id) === sheetGroupId
        }
  ...
  return (
    <>
      ...
      {!collapsed && hasUngrouped && (() => {
        ...
        return (
          <TableRow
            className={`${locked ? "cursor-not-allowed" : "cursor-pointer"} [&>td]:py-2.5 ${bgClass}`}
            onClick={() => { if (!locked) onViewSummary(null) }}
          >
            <TableCell>
              {isUgComplete ? (
